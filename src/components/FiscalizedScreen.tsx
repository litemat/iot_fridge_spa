...
import { Check, Unlock, QrCode, ArrowLeft } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useLanguage } from '../context/LanguageContext';

export function FiscalizedScreen() {
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          className="mb-12"
        >
          <div className="rounded-3xl bg-white p-8 inline-block" style={{ boxShadow: '0px 20px 60px rgba(0,0,0,0.12)' }}>
            <div className="mb-4 flex justify-center">
              <QrCode size={48} style={{ color: '#5E35B1' }} />
            </div>
            <p className="text-[#333333] mb-4 text-center" style={{ fontSize: '18px', fontWeight: '600' }}>
              {t('Фискальный чек', 'Фискалық чек')}
            </p>
            <div className="bg-gray-100 p-8 rounded-2xl">
              <div className="w-48 h-48 bg-gray-200 rounded-lg flex items-center justify-center">
                <span className="text-gray-600" style={{ fontSize: '14px', textAlign: 'center' }}>
                  {t('QR-код чека', 'QR-код чека')}
                </span>
              </div>
            </div>
          </div>
      ...
      <div
        className="absolute bottom-12 left-0 right-0 text-center text-[#666666]"
        style={{ fontSize: '14px' }}
      >
        {t('Автоматический возврат в меню...', 'Менюға автоматты оралу...')}
      </div>
    </div>
  );
}